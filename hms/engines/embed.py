...
class CharNGramEncoder:
    def __init__(self, dim: int = 1024, ngram_range=(2, 3)):
        self.dim = dim
...
class EmbeddingCache:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Create char-ngram encoder AFTER _ollama_dim is resolved from config
        self._char_encoder = CharNGramEncoder(dim=self._ollama_dim)
    ...
    def save_cache(self) -> None:
        ...
        with file_lock(self._cache_bin_path):
            # _char_encoder now uses _ollama_dim (1024) for consistency with Ollama vectors
            dim = self._ollama_dim
            import tempfile
