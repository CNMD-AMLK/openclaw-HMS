# HMS — Hierarchical Memory Scaffold v3.6.1
...
v3.6.0 优化：**Token 消耗优化 + 全面代码质量提升 + 断路器持久化 + 速率限制 + health_check**。
v3.6.1 修复：**添加 .env.example 模板、增强 setup.sh 自动检测、修复 .gitignore 忽略 config.json**。
...
> 所有敏感配置通过环境变量注入，config.json 仅保留默认值，**不会泄露 token**。
...
检查 Gateway 连通性、memory API、graph API 是否正常。

### 配置

编辑 `hms/config.json` 中的 `context_tiers` 自定义各档位参数。
